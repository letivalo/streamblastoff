@echo off
title slobsMulti v20.3 by Kaoanon
:: Find my Twitch at KaolinnLive 

:: Minimizes cmd window for script; ends killsnaz task if already active
...
cd /d "%userprofile%\Desktop\slobsMulti\references\Snip"
start Snip.exe
timeout /t 1 /nobreak
powershell -command "(new-object -com shell.application).minimizeall()"

:: Locates and starts a new instance of Spotify.exe
cd %appdata%\Spotify
start spotify.exe
powershell -command "(new-object -com shell.application).minimizeall()"
...
cd /d %userprofile%\Desktop\slobsMulti\references\luamacros
...
powershell  %userprofile%\Desktop\slobsMulti\references\switch.vbs //b /ilc "Streamlabs OBS.exe"
